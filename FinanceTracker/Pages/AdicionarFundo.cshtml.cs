...
{
    public class AdicionarFundoModel : PageModel
    {
        private readonly AtivoFinanceiroService _ativoService;
        private readonly FundoInvestimentoService _fundoService;

        public AdicionarFundoModel(AtivoFinanceiroService ativoService, FundoInvestimentoService fundoService)
        {
        ...
        [BindProperty]
        public FundoInvestimentoDto Fundo { get; set; } = new FundoInvestimentoDto();
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            Console.WriteLine($"- Nome: {Fundo.Nome}");
            Console.WriteLine($"- Montante: {Fundo.Montante}");
            Console.WriteLine($"- TaxaJuro: {Fundo.TaxaJuro}");
            ...
            Fundo.AtivoId = ativoCriado.Id;

            var fundoResult = await _fundoService.CreateFundo(Fundo);
            if (!fundoResult.ToLower().Contains("sucesso"))
            {
                ModelState.AddModelError(string.Empty, "Erro ao criar o depósito: " + fundoResult);
                return Page();
            }
